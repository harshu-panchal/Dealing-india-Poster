...
const TemplateManager = () => {
  ...
  // ── Business Card Field Designer ──
  const [showFieldDesigner, setShowFieldDesigner] = useState(false);
  const [fieldDesignerTemplate, setFieldDesignerTemplate] = useState(null);
  ...
  const handleOpenDesigner = (tpl) => {
    setFieldDesignerTemplate(tpl);
    setShowFieldDesigner(true);
  };

  const handleSaveFields = async (fields) => {
    if (!fieldDesignerTemplate?._id) return;
    try {
      const config = { headers: { Authorization: `Bearer ${admin.accessToken}` } };
      await axios.put(`${API_URL}/admin/templates/${fieldDesignerTemplate._id}`, { fields }, config);
      setShowFieldDesigner(false);
      setFieldDesignerTemplate(null);
      fetchTemplates();
    } catch (err) {
      alert('Failed to save field layout');
    }
  };
  ...
  return (
    <div className="space-y-10 pb-12 overflow-x-hidden">
            ...
            <Card className="border-none overflow-hidden hover:shadow-2xl transition-all duration-500 bg-white">
              <div className="aspect-[4/5] relative overflow-hidden bg-slate-100">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center gap-3 z-10">
                  ...
                  {tpl.subcategoryId?.name?.toLowerCase().includes('business card') && (
                    <Button
                      onClick={(e) => { e.stopPropagation(); handleOpenDesigner(tpl); }}
                      variant="outline" size="icon" className="h-10 w-10 bg-blue-500 text-white border-none"
                      title="Design Fields"
                    >
                      <Layers size={18} />
                    </Button>
                ...
                {tpl.fields?.length > 0 && (
                  <div className="mt-1.5 flex items-center gap-1">
                    ...
                    <span className="text-[8px] font-black text-blue-500 uppercase tracking-tighter">
                      {tpl.fields.length} fields defined
                    </span>
                  </div>
                )}
              </div>
      ...
      {/* ── BUSINESS CARD FIELD DESIGNER ── */}
      {showFieldDesigner && fieldDesignerTemplate && (
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
              ...
              <button
                ...
                className="w-9 h-9 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center border-none cursor-pointer transition-colors"
              >
                <X size={16} className="text-slate-500" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6">
              <BusinessCardFieldDesigner
                key={fieldDesignerTemplate._id}
                imageUrl={fieldDesignerTemplate.image}
                fields={fieldDesignerTemplate.fields || []}
                onSave={handleSaveFields}
                onClose={() => { setShowFieldDesigner(false); setFieldDesignerTemplate(null); }}
              />
            </div>
          </div>
        </div>
